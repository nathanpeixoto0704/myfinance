# Livro de Compras — Supabase + GitHub Pages
...
create policy "Permitir leitura" on compras
for select using (true);

-- Permite inserir
create policy "Permitir insercao" on compras
for insert with check (true);

-- Permite atualizar
create policy "Permitir atualizacao" on compras
for update using (true) with check (true);

-- Permite excluir
create policy "Permitir exclusao" on compras
...
tabela não for `compras`, ajuste `TABLE_NAME` em `config.js`.

## 2. Publicar no GitHub Pages

1. Crie um repositório novo no GitHub (ex: `livro-de-compras`)
2. Faça upload dos 4 arquivos (`index.html`, `styles.css`, `app.js`, `config.js`)
   ...
   git branch -M main
   git remote add origin https://github.com/SEU-USUARIO/livro-de-compras.git
   git push -u origin main
   ```
3. No repositório, vá em **Settings → Pages**
4. Em **Source**, selecione a branch `main` e a pasta `/ (root)`
5. Clique em **Save** — em 1-2 minutos seu site estará em:
   `https://SEU-USUARIO.github.io/livro-de-compras/`
